...
class KaliToolsClient:
    ...
    def safe_post(self, endpoint: str, json_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            logger.debug("POST %s with data: %s", url, json_data)
...
def main():
    ...
    if "error" in health:
        logger.warning("Unable to connect to Kali API server at %s: %s", args.server, health["error"])
        logger.warning("MCP server will start, but tool execution may fail")
    else:
        logger.info("Successfully connected to Kali API server at %s", args.server)
        logger.info("Server health status: %s", health["status"])
